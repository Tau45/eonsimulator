...
void EventCallServiceTermination::execute(Network &network, priority_queue<Event *, vector<Event *>, Event::EventComparator> &eventQueue, Generator &generator) {
	...
	if (connection.getPathSize() == 1) {
		Logger::instance().log(occurrenceTime, generator.getA(), generator.getSimulationIndex(), Logger::CONNECTION_CLOSED, "Connection between input: " + to_string(network.getInputLink(connection.getSourceLinkIndex())->getSourceNode()) + " and output " + to_string(network.getOutputLink(connection.getDestinationLinkIndex())->getDestinationNode()) + " closed: Freeing " + to_string(connection.getFirstFSUOfInputLink()) + "-" + to_string(connection.getFirstFSUOfInputLink() + connection.getRequiredNumberOfFSUs() - 1) + " (" + to_string(connection.getRequiredNumberOfFSUs()) + " FSUs)");
